...
public interface HMap {
     ...
     * If the map is modified while an iteration over the set is in progress, the results of the iteration 
     * are undefined. The set supports element removal, which removes the corresponding mapping from the map, 
	...
	HSet entrySet();
   	...
   	/**
   	 * Compares the specified object with this map for equality. Returns true if the given object is also a 
   	 * map and the two Maps represent the same mappings. More formally, two maps t1 and t2 represent the 
   	 * same mappings if t1.entrySet().equals(t2.entrySet()). This ensures that the equals method works 
	...
	boolean equals(Object o);
   	 ...
   	 * no mapping for this key. A return value of null does not necessarily indicate that the map 
     ...
     * of the hashCodes of each entry in the map's entrySet view. This ensures that t1.equals(t2) 
     ...
     * If the map previously contained a mapping for this key, the old value is replaced by 
     * the specified value. (A map m is said to contain a mapping for a key k if and only if 
     ...
     */
	Object put(Object key, Object value) throws ClassCastException, NullPointerException, IllegalArgumentException;
	
    /**
     * Copies all of the mappings from the specified map to this map (optional operation). The effect of this call 
     * is equivalent to that of calling put(k, v) on this map once for each mapping from key k to value v in the 
     * specified map. The behavior of this operation is unspecified if the specified map is modified while the 
     ...
     */
	void putAll(HMap t) throws ClassCastException, NullPointerException, IllegalArgumentException;
	
    /**
     * Removes the mapping for this key from this map if it is present (optional operation). More formally, if this map 
     * contains a mapping from key k to value v such that (key==null ? k==null : key.equals(k)), that mapping is removed. 
     * (The map can contain at most one such mapping.)
	 * Returns the value to which the map previously associated the key, or null if the map contained no mapping 
	 * for this key. (A null return can also indicate that the map previously associated null with the specified 
     ...
     */
	Object remove(Object key) throws ClassCastException, NullPointerException;
     ...
     * so changes to the map are reflected in the collection, and vice-versa. If the map is modified 
     * while an iteration over the collection is in progress, the results of the iteration are undefined. 
	...
	public static interface Entry {
		 ...
		 * the backing map (by the iterator's remove operation), the results of this call are undefined.
